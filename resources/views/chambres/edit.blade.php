...
<form action="{{ route('chambres.update', $chambre) }}" method="POST" class="bg-white p-6 rounded shadow max-w-lg">
        ...
        <select name="maison_id" id="maison_id"
            ...
            @foreach ($maisons as $maison)
                <option value="{{ $maison->id }}" 
                    {{ (old('maison_id') ?? $chambre->maison_id) == $maison->id ? 'selected' : '' }}>
                    {{ $maison->nom }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-4">
        ...
        <input type="text" name="code_chambre" id="code_chambre" 
            value="{{ old('code_chambre') ?? $chambre->code_chambre }}"
    ...
    <div class="mb-4">
        ...
        <input type="number" name="loyer_individuel" id="loyer_individuel" 
            value="{{ old('loyer_individuel') ?? $chambre->loyer_individuel }}"
    ...
    <div class="mb-4">
        ...
        <input type="number" name="caution" id="caution" min="0" step="0.01" 
            ...
            value="{{ old('caution') ?? $chambre->caution }}"
